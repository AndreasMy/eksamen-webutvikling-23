...
function createPost(event) { // CHANGELOG: Oppdatert denne funksjonen til å bruke event og rettet på credentials og reload av location
            ...
            data.forEach(element => {
                ...
                document.querySelector('#showPosts').innerHTML += `
                    <div class="box">
                        <h3>${element.title}</h3>
                        <p>${element.content}</p>
                        <p>Laget av ${element.username.charAt(0).toUpperCase() + element.username.slice(1)}</p>
                        <p>${formattedDate}</p>
                        <button onclick='showEditForm(${element.id})'>Edit</button>
